...
public static class AgentPathing
{
    ...
    // ✅ NEW: true if nothing blocks a straight line between the two tiles.
    // Lets a chaser (e.g. a fox after a pig) skip tile-based A* entirely and
    // steer straight at a moving target's live position whenever there's
    // nothing in the way — tile pathing quantizes to tile centers/borders,
    // which is too coarse to track something that moves continuously: it
    // can drift enough within a tile to be missed without ever crossing a
    // tile boundary, and re-pathing on a tile-cross is free to pick a
    // different route than last time, which reads as backtracking. Direct
    // pursuit has neither problem — it just re-aims every frame.
    public static bool HasClearLine(Agent agent, Vector2Int from, Vector2Int to)
    {
        if (from == to)
            return true;

        foreach (Vector2Int tile in GetTilesOnLine(from, to))
        {
            // the endpoints are allowed to be "blocked" — the mover's own
            // tile, and the target's tile (e.g. a pig standing inside its pen)
    ...
    // Bresenham line — every tile the straight line from `from` to `to`
    // actually passes through.
    private static IEnumerable<Vector2Int> GetTilesOnLine(Vector2Int from, Vector2Int to)
    {
